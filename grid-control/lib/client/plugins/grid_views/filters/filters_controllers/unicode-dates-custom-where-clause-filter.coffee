columnFilterStateToQuery = (column_filter_state, context) ->
  ranges_query = GridControl.UnicodeDatesFilterColumnFilterStateToQuery(column_filter_state, context)
  ranges = []
  ...
  query = {
    _id: {$ne: Math.random()} # EJSONing the query with the $where clause only will result in an empty object, since functions aren't being EJSONified.
                              ...
                              # invalidation.
    $where: context.column_schema_definition?.grid_column_filter_settings?.options?.dateRangeToWhereClauseFunction(ranges)
  }

  return query
  
GridControl.installFilterType "unicode-dates-custom-where-clause-filter",
  controller_constructor: GridControl.UnicodeDatesFilterControllerConstructor
  column_filter_state_to_query: columnFilterStateToQuery
  getSelectAllFilterState: GridControl.UnicodeDatesFilterGetSelectAllFilterState

_.extend JustdoCertMaintainer.prototype,
  registerTaskPaneSection: ->
    ...
    JustdoCertMaintainerTaskPaneSection = (options) ->
    ...
    # Note that we register this section under the "JustdoCertMaintainerTaskPaneSection" id
    # which we use later
    module.registerTaskPaneSection "JustdoCertMaintainerTaskPaneSection", JustdoCertMaintainerTaskPaneSection
    ...
    Util.inherits JustdoCertMaintainerTaskPaneSection, module.TaskPaneSection
    ...
    section_definition =
      id: "justdo-cert-maintainer"
      type: "JustdoCertMaintainerTaskPaneSection" # the name of the template derives from the type
      options:
        title: JustdoCertMaintainer.task_pane_tab_label
        titleInfo: -> "" # Can be a reactive resource, for number wrap with: <div class="task-pane-tab-title-info bg-primary text-white">0</div>
      section_options: {}

    justdo_cert_maintainer_section_position =
      lodash.findIndex task_pane_sections, (section) -> section.id == "item-activity"

    Tracker.autorun =>
      if APP.modules.project_page.curProj()?.isCustomFeatureEnabled(JustdoCertMaintainer.project_custom_feature_id)
        if not _.any(task_pane_sections, (section) => section.id == "justdo-cert-maintainer")
          if justdo_cert_maintainer_section_position != -1
            # If the item-activity section exist, put the file manager after it
            task_pane_sections.splice(justdo_cert_maintainer_section_position + 1, 0, section_definition)
          else
        ...
        _.find task_pane_sections, (section, i) =>
          if section.id == "justdo-cert-maintainer"
            index = i
            return true

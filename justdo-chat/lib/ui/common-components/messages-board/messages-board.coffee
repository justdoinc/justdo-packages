...
Template.common_chat_messages_board_message_card.onCreated ->
  # Pass the getChannelObject method to the message card template for ease of access
  if (getChannelObj = @closestInstance("common_chat_messages_board")?.data?.getChannelObject)?
    @getChannelObject = getChannelObj
  else
    @getChannelObject = ->
      return

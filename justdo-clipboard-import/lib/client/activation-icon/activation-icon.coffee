  ...
  for row in cp_data
    ...
    if not modal_data.rows_to_skip_set.get().has("#{row_index}")
      ...
      for column_num in [0..(number_of_columns - 1)]
        ...
        else if JustdoPlanningUtilities? and field_id == JustdoPlanningUtilities?.dependencies_mf_field_id
          if cell_val != ""
            dependencies_strs[task["jci:temp_import_id"]] = cell_val # XXX temp_import_id can be null
        else if JustdoPlanningUtilities? and field_id == JustdoPlanningUtilities?.task_duration_pseudo_field_id

...
JustdoDbMigrations.commonBatchedMigration = (options) ->
  ...
  migration_script_obj =
    runScript: ->
      pending_migration_set_cursor = options.collection.find(options.pending_migration_set_query, options.pending_migration_set_query_options)
      ...
      processBatch = =>
        ...
        num_processed += options.batchProcessor pending_migration_set_cursor, options.collection, options.custom_options

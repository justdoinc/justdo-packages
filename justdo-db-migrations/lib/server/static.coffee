...
JustdoDbMigrations.registerCronjob = (options) ->
  ...
  #
  # This function creates a simple cron job that runs a function on a schedule
  # without processing documents from a collection. It uses collection_less_mode
  # to avoid needing a collection/queryGenerator.
  ...
  register_db_cronjob_options = _.extend options,
    common_batch_migration_options:
      # Use collection_less_mode to run without querying a collection
      collection_less_mode: true

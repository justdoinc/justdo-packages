_.extend JustdoDeliveryPlanner.prototype,
  registerTaskPaneSection: ->
    ...
    JustdoDeliveryPlannerTaskPaneSection = (options) ->
    ...
    # Note that we register this section under the "JustdoDeliveryPlannerTaskPaneSection" id
    # which we use later
    module.registerTaskPaneSection "JustdoDeliveryPlannerTaskPaneSection", JustdoDeliveryPlannerTaskPaneSection
    ...
    Util.inherits JustdoDeliveryPlannerTaskPaneSection, module.TaskPaneSection
    ...
    Tracker.autorun =>
      if APP.modules.project_page.curProj()?.isCustomFeatureEnabled(JustdoDeliveryPlanner.project_custom_feature_id)
        if not _.any(task_pane_sections, (section) => section.id == "justdo-delivery-planner")
          if justdo_delivery_planner_section_position != -1
            # If the item-activity section exist, put the file manager after it
            task_pane_sections.splice(justdo_delivery_planner_section_position + 1, 0, section_definition)
          else
        ...
        _.find task_pane_sections, (section, i) =>
          if section.id == "justdo-delivery-planner"
            index = i
            return true

_.extend JustdoGantt.prototype,
  registerTaskPaneSection: ->
    ...
    JustdoGanttTaskPaneSection = (options) ->
    ...
    # Note that we register this section under the "JustdoGanttTaskPaneSection" id
    # which we use later
    module.registerTaskPaneSection "JustdoGanttTaskPaneSection", JustdoGanttTaskPaneSection
    ...
    Util.inherits JustdoGanttTaskPaneSection, module.TaskPaneSection
    ...
    section_definition =
      id: "justdo-gantt"
      type: "JustdoGanttTaskPaneSection" # the name of the template derives from the type
      options:
        title: JustdoGantt.task_pane_tab_label
        titleInfo: -> "" # Can be a reactive resource
      section_options: {}

    justdo_gantt_section_position =
      lodash.findIndex task_pane_sections, (section) -> section.id == "item-activity"

    Tracker.autorun =>
      if APP.modules.project_page.curProj()?.isCustomFeatureEnabled(JustdoGantt.project_custom_feature_id)
        if not _.any(task_pane_sections, (section) => section.id == "justdo-gantt")
          if justdo_gantt_section_position != -1
            # If the item-activity section exist, put the file manager after it
            task_pane_sections.splice(justdo_gantt_section_position + 1, 0, section_definition)
          else

...
# APP.justdo_global_activity_log = new JustdoGlobalActivityLog()
...
APP.getEnv (env) ->
  options =
    ...
    tasks_changelog_collection: APP.collections.TasksChangelog

  # If an env variable affect this package load, check its value here
  # remember env vars are Strings
  APP.justdo_global_activity_log = new JustdoGlobalActivityLog(options)
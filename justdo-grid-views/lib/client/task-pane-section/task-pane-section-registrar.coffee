_.extend JustdoGridViews.prototype,
  registerTaskPaneSection: ->
    ...
    JustdoGridViewsTaskPaneSection = (options) ->
    ...
    # Note that we register this section under the "JustdoGridViewsTaskPaneSection" id
    # which we use later
    module.registerTaskPaneSection "JustdoGridViewsTaskPaneSection", JustdoGridViewsTaskPaneSection
    ...
    Util.inherits JustdoGridViewsTaskPaneSection, module.TaskPaneSection
    ...
    section_definition =
      id: "justdo-grid-views"
      type: "JustdoGridViewsTaskPaneSection" # the name of the template derives from the type
      options:
        title: JustdoGridViews.task_pane_tab_label
        titleInfo: -> "" # Can be a reactive resource, for number wrap with: <div class="task-pane-tab-title-info bg-primary text-white">0</div>
      section_options: {}

    justdo_grid_views_section_position =
      lodash.findIndex task_pane_sections, (section) -> section.id == "item-activity"

    Tracker.autorun =>
      if APP.modules.project_page.curProj()?.isCustomFeatureEnabled(JustdoGridViews.project_custom_feature_id)
        if not _.any(task_pane_sections, (section) => section.id == "justdo-grid-views")
          if justdo_grid_views_section_position != -1
            # If the item-activity section exist, put the file manager after it
            task_pane_sections.splice(justdo_grid_views_section_position + 1, 0, section_definition)
          else
        ...
        _.find task_pane_sections, (section, i) =>
          if section.id == "justdo-grid-views"
            index = i
            return true

_.extend JustdoI18nRoutes.prototype,
  registerTaskPaneSection: ->
    ...
    JustdoI18nRoutesTaskPaneSection = (options) ->
    ...
    # Note that we register this section under the "JustdoI18nRoutesTaskPaneSection" id
    # which we use later
    project_page_module.registerTaskPaneSection "JustdoI18nRoutesTaskPaneSection", JustdoI18nRoutesTaskPaneSection
    ...
    Util.inherits JustdoI18nRoutesTaskPaneSection, project_page_module.TaskPaneSection
    ...
    section_definition =
      id: "justdo-i18n-routes"
      type: "JustdoI18nRoutesTaskPaneSection" # the name of the template derives from the type
      options:
        title: JustdoI18nRoutes.task_pane_tab_label
        titleInfo: -> "" # Can be a reactive resource, for number wrap with: <div class="task-pane-tab-title-info bg-primary text-white">0</div>
      section_options: {}

    justdo_i18n_routes_section_position =
      lodash.findIndex task_pane_sections, (section) -> section.id == "item-activity"

    Tracker.autorun =>
      if APP.modules.project_page.curProj()?.isCustomFeatureEnabled(JustdoI18nRoutes.project_custom_feature_id)
        if not _.any(task_pane_sections, (section) => section.id == "justdo-i18n-routes")
          if justdo_i18n_routes_section_position != -1
            # If the item-activity section exist, put the file manager after it
            task_pane_sections.splice(justdo_i18n_routes_section_position + 1, 0, section_definition)
          else
        ...
        _.find task_pane_sections, (section, i) =>
          if section.id == "justdo-i18n-routes"
            index = i
            return true

_.extend JustdoI18n.prototype,
  registerTaskPaneSection: ->
    ...
    JustdoI18nTaskPaneSection = (options) ->
    ...
    # Note that we register this section under the "JustdoI18nTaskPaneSection" id
    # which we use later
    module.registerTaskPaneSection "JustdoI18nTaskPaneSection", JustdoI18nTaskPaneSection
    ...
    Util.inherits JustdoI18nTaskPaneSection, module.TaskPaneSection
    ...
    section_definition =
      id: "justdo-i18n"
      type: "JustdoI18nTaskPaneSection" # the name of the template derives from the type
      options:
        title: JustdoI18n.task_pane_tab_label
        titleInfo: -> "" # Can be a reactive resource, for number wrap with: <div class="task-pane-tab-title-info bg-primary text-white">0</div>
      section_options: {}

    justdo_i18n_section_position =
      lodash.findIndex task_pane_sections, (section) -> section.id == "item-activity"

    Tracker.autorun =>
      if APP.modules.project_page.curProj()?.isCustomFeatureEnabled(JustdoI18n.project_custom_feature_id)
        if not _.any(task_pane_sections, (section) => section.id == "justdo-i18n")
          if justdo_i18n_section_position != -1
            # If the item-activity section exist, put the file manager after it
            task_pane_sections.splice(justdo_i18n_section_position + 1, 0, section_definition)
          else

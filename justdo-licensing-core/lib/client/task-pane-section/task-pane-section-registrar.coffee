_.extend JustdoLicensingCore.prototype,
  registerTaskPaneSection: ->
    ...
    JustdoLicensingCoreTaskPaneSection = (options) ->
    ...
    # Note that we register this section under the "JustdoLicensingCoreTaskPaneSection" id
    # which we use later
    module.registerTaskPaneSection "JustdoLicensingCoreTaskPaneSection", JustdoLicensingCoreTaskPaneSection
    ...
    Util.inherits JustdoLicensingCoreTaskPaneSection, module.TaskPaneSection
    ...
    section_definition =
      id: "justdo-licensing-core"
      type: "JustdoLicensingCoreTaskPaneSection" # the name of the template derives from the type
      options:
        title: JustdoLicensingCore.task_pane_tab_label
        titleInfo: -> "" # Can be a reactive resource, for number wrap with: <div class="task-pane-tab-title-info bg-primary text-white">0</div>
      section_options: {}

    justdo_licensing_core_section_position =
      lodash.findIndex task_pane_sections, (section) -> section.id == "item-activity"

    Tracker.autorun =>
      if APP.modules.project_page.curProj()?.isCustomFeatureEnabled(JustdoLicensingCore.project_custom_feature_id)
        if not _.any(task_pane_sections, (section) => section.id == "justdo-licensing-core")
          if justdo_licensing_core_section_position != -1
            # If the item-activity section exist, put the file manager after it
            task_pane_sections.splice(justdo_licensing_core_section_position + 1, 0, section_definition)
          else
        ...
        _.find task_pane_sections, (section, i) =>
          if section.id == "justdo-licensing-core"
            index = i
            return true

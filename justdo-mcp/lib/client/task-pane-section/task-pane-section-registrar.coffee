_.extend JustdoMcp.prototype,
  registerTaskPaneSection: ->
    ...
    JustdoMcpTaskPaneSection = (options) ->
    ...
    # Note that we register this section under the "JustdoMcpTaskPaneSection" id
    # which we use later
    project_page_module.registerTaskPaneSection "JustdoMcpTaskPaneSection", JustdoMcpTaskPaneSection
    ...
    Util.inherits JustdoMcpTaskPaneSection, project_page_module.TaskPaneSection
    ...
    section_definition =
      id: "justdo-mcp"
      type: "JustdoMcpTaskPaneSection" # the name of the template derives from the type
      options:
        title: JustdoMcp.task_pane_tab_label
        titleInfo: -> "" # Can be a reactive resource, for number wrap with: <div class="task-pane-tab-title-info bg-primary text-white">0</div>
      section_options: {}

    justdo_mcp_section_position =
      lodash.findIndex task_pane_sections, (section) -> section.id == "item-activity"

    Tracker.autorun =>
      if APP.modules.project_page.curProj()?.isCustomFeatureEnabled(JustdoMcp.project_custom_feature_id)
        if not _.any(task_pane_sections, (section) => section.id == "justdo-mcp")
          if justdo_mcp_section_position != -1
            # If the item-activity section exist, put the file manager after it
            task_pane_sections.splice(justdo_mcp_section_position + 1, 0, section_definition)
          else

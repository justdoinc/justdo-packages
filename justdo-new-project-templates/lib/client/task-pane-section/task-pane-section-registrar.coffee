_.extend JustdoNewProjectTemplates.prototype,
  registerTaskPaneSection: ->
    ...
    JustdoNewProjectTemplatesTaskPaneSection = (options) ->
    ...
    # Note that we register this section under the "JustdoNewProjectTemplatesTaskPaneSection" id
    # which we use later
    module.registerTaskPaneSection "JustdoNewProjectTemplatesTaskPaneSection", JustdoNewProjectTemplatesTaskPaneSection
    ...
    Util.inherits JustdoNewProjectTemplatesTaskPaneSection, module.TaskPaneSection
    ...
    section_definition =
      id: "justdo-new-project-templates"
      type: "JustdoNewProjectTemplatesTaskPaneSection" # the name of the template derives from the type
      options:
        title: JustdoNewProjectTemplates.task_pane_tab_label
        titleInfo: -> "" # Can be a reactive resource, for number wrap with: <div class="task-pane-tab-title-info bg-primary text-white">0</div>
      section_options: {}

    justdo_new_project_templates_section_position =
      lodash.findIndex task_pane_sections, (section) -> section.id == "item-activity"

    Tracker.autorun =>
      if APP.modules.project_page.curProj()?.isCustomFeatureEnabled(JustdoNewProjectTemplates.project_custom_feature_id)
        if not _.any(task_pane_sections, (section) => section.id == "justdo-new-project-templates")
          if justdo_new_project_templates_section_position != -1
            # If the item-activity section exist, put the file manager after it
            task_pane_sections.splice(justdo_new_project_templates_section_position + 1, 0, section_definition)
          else
        ...
        _.find task_pane_sections, (section, i) =>
          if section.id == "justdo-new-project-templates"
            index = i
            return true

_.extend JustdoNews.prototype,
  registerTaskPaneSection: ->
    ...
    JustdoNewsTaskPaneSection = (options) ->
    ...
    # Note that we register this section under the "JustdoNewsTaskPaneSection" id
    # which we use later
    module.registerTaskPaneSection "JustdoNewsTaskPaneSection", JustdoNewsTaskPaneSection
    ...
    Util.inherits JustdoNewsTaskPaneSection, module.TaskPaneSection
    ...
    section_definition =
      id: "justdo-news"
      type: "JustdoNewsTaskPaneSection" # the name of the template derives from the type
      options:
        title: JustdoNews.task_pane_tab_label
        titleInfo: -> "" # Can be a reactive resource, for number wrap with: <div class="task-pane-tab-title-info bg-primary text-white">0</div>
      section_options: {}

    justdo_news_section_position =
      lodash.findIndex task_pane_sections, (section) -> section.id == "item-activity"

    Tracker.autorun =>
      if APP.modules.project_page.curProj()?.isCustomFeatureEnabled(JustdoNews.project_custom_feature_id)
        if not _.any(task_pane_sections, (section) => section.id == "justdo-news")
          if justdo_news_section_position != -1
            # If the item-activity section exist, put the file manager after it
            task_pane_sections.splice(justdo_news_section_position + 1, 0, section_definition)
          else

_.extend JustdoProjectsDashboard.prototype,
  registerTaskPaneSection: ->
    ...
    JustdoProjectsDashboardTaskPaneSection = (options) ->
    ...
    # Note that we register this section under the "JustdoProjectsDashboardTaskPaneSection" id
    # which we use later
    module.registerTaskPaneSection "JustdoProjectsDashboardTaskPaneSection", JustdoProjectsDashboardTaskPaneSection
    ...
    Util.inherits JustdoProjectsDashboardTaskPaneSection, module.TaskPaneSection
    ...
    section_definition =
      id: "justdo-projects-dashboard"
      type: "JustdoProjectsDashboardTaskPaneSection" # the name of the template derives from the type
      options:
        title: JustdoProjectsDashboard.task_pane_tab_label
        titleInfo: -> "" # Can be a reactive resource
      section_options: {}

    justdo_projects_dashboard_section_position =
      lodash.findIndex task_pane_sections, (section) -> section.id == "item-activity"

    Tracker.autorun =>
      if APP.modules.project_page.curProj()?.isCustomFeatureEnabled(JustdoProjectsDashboard.project_custom_feature_id)
        if not _.any(task_pane_sections, (section) => section.id == "justdo-projects-dashboard")
          if justdo_projects_dashboard_section_position != -1
            # If the item-activity section exist, put the file manager after it
            task_pane_sections.splice(justdo_projects_dashboard_section_position + 1, 0, section_definition)
          else
        ...
        _.find task_pane_sections, (section, i) =>
          if section.id == "justdo-projects-dashboard"
            index = i
            return true

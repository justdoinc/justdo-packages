_.extend JustdoResourcesAvailability.prototype,
  registerTaskPaneSection: ->
    ...
    JustdoResourcesAvailabilityTaskPaneSection = (options) ->
    ...
    # Note that we register this section under the "JustdoResourcesAvailabilityTaskPaneSection" id
    # which we use later
    project_page_module.registerTaskPaneSection "JustdoResourcesAvailabilityTaskPaneSection", JustdoResourcesAvailabilityTaskPaneSection
    ...
    Util.inherits JustdoResourcesAvailabilityTaskPaneSection, project_page_module.TaskPaneSection
    ...
    section_definition =
      id: "justdo-resources-availability"
      type: "JustdoResourcesAvailabilityTaskPaneSection" # the name of the template derives from the type
      options:
        title: JustdoResourcesAvailability.task_pane_tab_label
        titleInfo: -> "" # Can be a reactive resource
      section_options: {}

    justdo_resources_availability_section_position =
      lodash.findIndex task_pane_sections, (section) -> section.id == "item-activity"

    Tracker.autorun =>
      if APP.modules.project_page.curProj()?.isCustomFeatureEnabled(JustdoResourcesAvailability.project_custom_feature_id)
        if not _.any(task_pane_sections, (section) => section.id == "justdo-resources-availability")
          if justdo_resources_availability_section_position != -1
            # If the item-activity section exist, put the file manager after it
            task_pane_sections.splice(justdo_resources_availability_section_position + 1, 0, section_definition)
          else
        ...
        _.find task_pane_sections, (section, i) =>
          if section.id == "justdo-resources-availability"
            index = i
            return true

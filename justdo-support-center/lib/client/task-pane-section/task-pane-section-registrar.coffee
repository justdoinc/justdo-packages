_.extend JustdoSupportCenter.prototype,
  registerTaskPaneSection: ->
    ...
    JustdoSupportCenterTaskPaneSection = (options) ->
    ...
    # Note that we register this section under the "JustdoSupportCenterTaskPaneSection" id
    # which we use later
    project_page_module.registerTaskPaneSection "JustdoSupportCenterTaskPaneSection", JustdoSupportCenterTaskPaneSection
    ...
    Util.inherits JustdoSupportCenterTaskPaneSection, project_page_module.TaskPaneSection
    ...
    section_definition =
      id: "justdo-support-center"
      type: "JustdoSupportCenterTaskPaneSection" # the name of the template derives from the type
      options:
        title: JustdoSupportCenter.task_pane_tab_label
        titleInfo: -> "" # Can be a reactive resource, for number wrap with: <div class="task-pane-tab-title-info bg-primary text-white">0</div>
      section_options: {}

    justdo_support_center_section_position =
      lodash.findIndex task_pane_sections, (section) -> section.id == "item-activity"

    Tracker.autorun =>
      if APP.modules.project_page.curProj()?.isCustomFeatureEnabled(JustdoSupportCenter.project_custom_feature_id)
        if not _.any(task_pane_sections, (section) => section.id == "justdo-support-center")
          if justdo_support_center_section_position != -1
            # If the item-activity section exist, put the file manager after it
            task_pane_sections.splice(justdo_support_center_section_position + 1, 0, section_definition)
          else
        ...
        _.find task_pane_sections, (section, i) =>
          if section.id == "justdo-support-center"
            index = i
            return true

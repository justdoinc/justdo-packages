APP.executeAfterAppLibCode ->
  ...
  # Note: During the phase we add multi-grid support to nullary operations, this "zooomIn" op still uses the main gcm
  # because we didn't have multi gcm support yet.
  # In the future when multi gcm support is added, the `op` and `prereq` will be updated to use the gcm provided as param.

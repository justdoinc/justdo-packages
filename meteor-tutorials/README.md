...
- `spot`: (*optional*) jQuery selector of elements to highlight (can be a single selector or separated by commas). If multiple elements are selected, the tutorial automatically calculates a box that will fit around all of them. JustDo fork: spot can also be a reactive function that returns the spot selector string.
- `only_one_spot`: (*optional*) JustDo fork: if there is more than one element matching the spot selector - take the first one
- `onLoad`: (*optional*) a function that will run whenever the tutorial hits this step. Helpful if you need to make sure your interface is in a certain state before displaying the tutorial contents. JustDo fork: First argument to this function will be the tutorial manager object.
...
- `nextStepTrigger`: (*optional*) JustDo fork: A reactive function, we'll move to the next step automatically once this function will return true. Important! if this function returns true on step init - the step will be skipped without the user noticing it.
- `show_next_button`: (*optional*) JustDo fork: A Boolean, we show the next/finish button only if set to true. (default: false)
- `show_next_button`: (*optional*) JustDo fork: A Boolean, we show the prev button only if set to true. (default: false)

...
# APP.tasks_file_manager_plugin = new TasksFileManagerPlugin()
...
APP.getEnv (env) ->
  if env.TASKS_FILES_UPLOAD_ENABLED == "true"
    if not _.isEmpty(env.FILESTACK_KEY)
      APP.executeAfterAppLibCode ->
        APP.tasks_file_manager_plugin = new TasksFileManagerPlugin()
    else
      APP.logger.debug "Not enabling tasks-file-manager because api key is not set (env.FILESTACK_KEY is null)."